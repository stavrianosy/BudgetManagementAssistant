...
namespace CustomWrapPanel
{
    public class WrapPanel : Panel
    {
        ...
        {
            //Create available size for child control
            //In Horizontal orientation child control can have a maximum width of BlockSize
            //And it's width can be "unlimited" - I want to know what height would control like to have at given height
            Size childAvailableSize = new Size(double.PositiveInfinity, BlockSize);

            //Next, i want to stack my child controls under each other (i call it block), until i reach my available width. 
            //From that point i want to begin another block of controls next to the current one.
            ...
            var remainingSpace = availableSize.Width; //Set my limit as my available width.
            foreach (var item in Children)
            {
                item.Measure(childAvailableSize); //Let the child measure itself (result of this will be in item.DesiredSize
                if (item.DesiredSize.Width > remainingSpace) //If there is not enough space for this control
                {
                    //Then we will start a new block, but only if the current block is not empty
                    //if its empty, then remaining space will be equal to available width.
                ...
                //In any case, decrease remaining space by desired width of control.
                remainingSpace -= item.DesiredSize.Width;
            }

            //Now we need to report back how much size we want,
            //thats number of blocks * their heigth, plus spaces between blocks
            //And for width, we will take what ever we can get.
            Size desiredSize = new Size();
            if (blockCount > 0)
            {
        ...
        {
            //Each child control will be placed in rectangle with height of BlockSize
            //and width of child controls desired width.
            //Upper left corner of first controls rectangle will initialy start at 0,0 relative to this control
            //and move down by width of control and more to the top by BlockSize once the block runs out of free space
            double offsetX = 0;
            double offsetY = 0;
            foreach (var item in Children)
            {
                ...
                {
                    if (offsetX != 0) //and the current block is not empty. (same rules as in measureoverride)
                    {
                        offsetY += BlockSpacing; //We will increse offset from top by the block size
                        offsetY += BlockSize;    //and spacing between blocks
                        offsetX = 0;             //and finally reset offset from left
                    }
                }
                //Create rectangle for child control
