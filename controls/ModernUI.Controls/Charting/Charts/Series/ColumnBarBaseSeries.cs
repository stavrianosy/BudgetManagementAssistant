...
namespace ModernUI.Toolkit.Data.Charting.Charts.Series
{
        ...
        where T : DataPoint, new()
    {
        ...
        {
            Type ColumnType = typeof(ColumnSeries);

            // redraw all other column series to ensure they make space for new one
            foreach (Series series in seriesHost.Series)
            {
                if (series.GetType() == ColumnType)
                {

                    ((ColumnBarBaseSeries<T>)series).UpdateDataPoints(((ColumnBarBaseSeries<T>)series).ActiveDataPoints);
                }
            }
        }
