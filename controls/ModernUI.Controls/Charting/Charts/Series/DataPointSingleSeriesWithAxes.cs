...
namespace ModernUI.Toolkit.Data.Charting.Charts.Series
{
    ...
    public abstract class DataPointSingleSeriesWithAxes : DataPointSeriesWithAxes, IRequireGlobalSeriesIndex
    {
        ...
        protected virtual void OnGlobalSeriesIndexPropertyChanged(int? oldValue, int? newValue)
        {
            if (!CustomTitleInUse)
            {
                //Title = newValue.HasValue ? string.Format(CultureInfo.CurrentCulture, ModernUI.Toolkit.Data.Charting.Resources.Series_OnGlobalSeriesIndexPropertyChanged_UntitledSeriesFormatString, newValue.Value + 1) : null;
